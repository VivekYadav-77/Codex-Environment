...
const commandForLanguage = (language) => {
    ...
    if (language === 'python') return { command: 'python', extension: 'py' }
    return null
}

export async function runJudgedSubmission({ code, language, question }) {
    ...
    if (!runtime) {
        return {
            ...
            runtimeMs: 0,
            testResults: [],
            error: `Language ${language} is not supported in V1`,
        }
    }
