...
const newEntries = [
    {
        ...
        "operations": ["check-overflow", "update-rear", "insert-element"],
        "code": {
        ...
        "practice": {
            "theory": ["What is Queue Overflow?", "How does Rear change on Enqueue?"],
    ...
    },
    {
        ...
        "operations": ["check-underflow", "access-front", "update-front"],
        "code": {
        ...
        "practice": {
            "theory": ["What is Queue Underflow?", "How does Front change on Dequeue?"],
...
try {
    ...
    if (!raw.endsWith(']')) throw new Error('File does not end with ]');

    // Check if they already exist
    if (raw.includes('queue-enqueue') || raw.includes('queue-dequeue')) {
        console.log('Entries already exist.');
        process.exit(0);
    }

    const json = JSON.parse(raw);
    json.push(...newEntries);
