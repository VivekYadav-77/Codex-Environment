...
try {
    ...
    // Filter out existing ones if they somehow got added but we can't see them
    const existingIds = new Set(data.map(a => a.id));
    const filteredNew = newAlgos.filter(a => !existingIds.has(a.id));

    if (filteredNew.length > 0) {
        data = data.concat(filteredNew);
        fs.writeFileSync(filePath, JSON.stringify(data, null, 4), 'utf8');
        console.log(`Successfully added ${filteredNew.length} new algorithms.`);
    } else {
